...
`main` and `develop` are trunk branches — they do not use a prefix. Never create new branches with the same names as trunk branches; branch off them instead.
...
If that returns nothing, check which trunk branch exists locally:

```bash
git show-ref --verify --quiet refs/heads/main && echo "main" || echo "develop"
```
